    ...
    with open('./stats_test/bleu.txt','w') as fb, open('./stats_test/xMatch.txt','w') as fm, open('./stats_test/codebleu.txt','w') as fr:
        for i in range(num_batches):
            try:
                predictions=[]
                accs = []
                with open(os.path.join('./stats_test/files/','sample_batch_{}.output'.format(str(i))),'w') as f_out, open(os.path.join('./stats_test/files/','sample_batch_{}.gold'.format(str(i))),'w') as f_gold, open(os.path.join('./stats_test/files/','sample_batch_{}.src'.format(str(i))),'w') as f_src:
                    if i != (num_batches-1):
                    ...
                    for ref,gold in zip(p_batch,eval_examples_batch):
                        ...
                        f_out.write(ref.strip() + '\n')
                        f_gold.write(gold.target.strip() + '\n')
                        f_src.write(gold.source.strip() + '\n')
                        accs.append(ref.strip()==gold.target.strip())
                ...
                codebleu = calc_code_bleu.get_codebleu(gold_fn, output_fn, args.lang)*100
