...
def eval_bleu(args, dev_dataset, model, device, tokenizer):
    ...
    codebleu = calc_code_bleu.get_codebleu(gold_fn, output_fn, 'java')*100
    xMatch = round(np.mean(accs)*100,4)

    return dev_bleu, xMatch, codebleu
...
def test(args, model, tokenizer, device, epoch=0):
    ...
    dev_bleu=round(bleu.bleuFromMaps(goldMap, predictionMap)[0],2)
    gold_fn = os.path.join(args.output_dir,"test_{}.gold".format(str(epoch)))
    output_fn = os.path.join(args.output_dir,"test_{}.output".format(str(epoch)))
    codebleu = calc_code_bleu.get_codebleu(gold_fn, output_fn, 'java')*100
    xMatch = round(np.mean(accs)*100,4)
    
    logger.info("  %s = %s, %s = %s, %s = %s"%("bleu-4",str(dev_bleu), "xMatch", str(xMatch), "codebleu", str(codebleu)))
    logger.info("  "+"*"*20)
    ...
    with open('./stats_test/bleu.txt','w') as fb, open('./stats_test/xMatch.txt','w') as fm, open('./stats_test/codebleu.txt','w') as fr:
        for i in range(num_batches):
            try:
                ...
                dev_bleu = round(bleu.bleuFromMaps(goldMap, predictionMap)[0],2)
                gold_fn = os.path.join('./stats_test/files/','sample_batch_{}.gold'.format(str(i)))
                output_fn = os.path.join('./stats_test/files/','sample_batch_{}.output'.format(str(i)))
                codebleu = calc_code_bleu.get_codebleu(gold_fn, output_fn, 'java')*100
                xMatch = round(np.mean(accs)*100,4)
                fb.write(str(dev_bleu)+'\n')
                fm.write(str(xMatch)+'\n')
                fr.write(str(codebleu)+'\n')
            except:
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval:
                ...
                dev_bleu, xMatch, codebleu  = eval_bleu(args, dev_dataset, model, device, tokenizer)
                logger.info("  %s = %s, %s = %s, %s = %s"%("bleu-4",str(dev_bleu), "xMatch", str(xMatch), "codebleu", str(codebleu)))
                logger.info("  "+"*"*20)    
