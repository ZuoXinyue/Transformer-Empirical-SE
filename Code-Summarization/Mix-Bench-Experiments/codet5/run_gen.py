...
def eval_bleu_epoch(args, eval_data, eval_examples, model, tokenizer, split_tag, criteria):
    ...
    pred_ids = []
    # change
    bleu, codebleu, meteor, rouge = 0.0, 0.0, 0.0, 0.0
    total_memory = 0
    total_cpu_usage = 0
    ...
    if args.task in ['defect']:
        ...
        if args.task == 'summarize':
            ...
            list_candidates = []
            for key in goldMap:
                if key in predictionMap:
                    list_references.append(goldMap[key][0].split(' '))
                    list_candidates.append(predictionMap[key][0].split(' '))
                else:
                    print('Missing key: ' + str(key))
            meteor=round(meteor_calculator.compute_score(list_references, list_candidates)[0]*100,2)
            rouge_calculator = Rouge()
            rouge=round(rouge_calculator.compute_score(list_references, list_candidates)[0]*100,2)
        else:
        ...
        # change
        result = {'em': np.mean(dev_accs) * 100, 'bleu': bleu, 'meteor': meteor, 'rouge': rouge}
        if args.task in ['concode', 'translate', 'refine']:
    ...
    num_batches = 20
    # CLT: sample size of 
    num_examples_per_batch = len(eval_examples)//num_batches
    with open('./stats_test/bleu.txt','w') as fb, open('./stats_test/meteor.txt','w') as fm, open('./stats_test/rouge.txt','w') as fr:
        for i in range(num_batches):
            try:
                predictions=[]
                with open(os.path.join('./stats_test/files/','sample_batch_{}.output'.format(str(i))),'w') as f_out, open(os.path.join('./stats_test/files/','sample_batch_{}.gold'.format(str(i))),'w') as f_gold:
                    if i != (num_batches-1):
                        p_batch = pred_nls[i*num_examples_per_batch:(i+1)*num_examples_per_batch]
                        eval_examples_batch = eval_examples[i*num_examples_per_batch:(i+1)*num_examples_per_batch]
                    else:
                        p_batch = pred_nls[i*num_examples_per_batch:]
                        eval_examples_batch = eval_examples[i*num_examples_per_batch:]
                    for ref,gold in zip(p_batch,eval_examples_batch):
                        predictions.append(str(gold.idx)+'\t'+ref)
                        f_out.write(str(gold.idx)+'\t'+ref.strip()+'\n')
                        f_gold.write(str(gold.idx)+'\t'+gold.target.strip()+'\n')

                (goldMap, predictionMap) = smooth_bleu.computeMaps(predictions, os.path.join('./stats_test/files/','sample_batch_{}.gold'.format(str(i))))
                ...
                for key in goldMap:
                    if key in predictionMap:
                        list_references.append(goldMap[key][0].split(' '))
                        list_candidates.append(predictionMap[key][0].split(' '))
                dev_meteor=round(meteor_calculator.compute_score(list_references, list_candidates)[0]*100,2)
                rouge_calculator = Rouge()
                dev_rouge=round(rouge_calculator.compute_score(list_references, list_candidates)[0]*100,2)
                fb.write(str(dev_bleu)+'\n')
                fm.write(str(dev_meteor)+'\n')
                fr.write(str(dev_rouge)+'\n')
            except:
                fb.write('An Error Has Occurred.'+'\n')
                fm.write('An Error Has Occurred.'+'\n')
                fr.write('An Error Has Occurred.'+'\n')
...
def main():
        ...
        for cur_epoch in range(args.start_epoch, int(args.num_train_epochs)):
            ...
            if args.do_eval:
                ...
                if args.do_eval_bleu:
                    ...
                    # change
                    dev_bleu, dev_em, dev_meteor, dev_rouge = result['bleu'], result['em'], result['meteor'], result['rouge']
                    if args.task in ['summarize']:
    ...
    if args.do_test:
        ...
        for criteria in ['best-bleu']:
            ...
            test_bleu, test_em, test_meteor, test_rouge = result['bleu'], result['em'], result['meteor'], result['rouge']
            test_codebleu = result['codebleu'] if 'codebleu' in result else 0
            result_str = "[%s] bleu-4: %.2f, em: %.4f, codebleu: %.4f\n" % (criteria, test_bleu, test_em, test_codebleu)
            logger.info(result_str)
            logger.info("  %s = %s "%("meteor",str(test_meteor)))
            logger.info("  %s = %s "%("rouge-l",str(test_rouge)))
            fa.write(result_str)
            if args.res_fn:
