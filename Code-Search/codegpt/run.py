...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        bar = tqdm(train_dataloader,total=len(train_dataloader)) # adding tqdm for visualization if this round of result is not ideal
        tr_num=0
        train_loss=0
...
def test(args, model, tokenizer):
    ...
    print("reached calculating scores")
    scores=np.matmul(nl_vecs,code_vecs.T)
...
def main():
    ...
    if args.do_test and args.local_rank in [-1, 0]:
        checkpoint_prefix = 'checkpoint-best-mrr/model.bin'
        print("reached doing test")
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
        model.load_state_dict(torch.load(output_dir))                  
        model.to(args.device)
        print("reached loading model")
